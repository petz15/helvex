...
def semantic_search(
    ...
    Results are sorted by cosine similarity to the query. Each category type
    returns up to top_k matches with their company count and similarity score.
    This enables 'find me companies in green energy consulting' style discovery
    across all categorisation dimensions simultaneously.
    """
    from app.services.embeddings import embed_single, nearest_neighbours
    import numpy as np
    ...
    def _score_items(items: list[tuple[str, int]], max_items: int = 200) -> list[dict]:
        """Embed item labels and rank by cosine similarity to query."""
        if not items:
            return []
        labels = [item[0] for item in items[:max_items]]
        counts = {item[0]: item[1] for item in items[:max_items]}
        try:
            item_vecs = embed_single.__func__ if hasattr(embed_single, '__func__') else None
