...
def list_notes(
    ...
    return crud.list_notes_for_company(db, company_id, user_id=current_user.id, org_id=current_user.org_id)
...
def create_note(
    ...
    if not crud.get_company(db, company_id):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Company not found")
    return crud.create_note(db, company_id, note_in, user_id=current_user.id, org_id=current_user.org_id)
...
def get_note(
    ...
    # Enforce ownership: only the author or a superadmin can view a private note
    if not current_user.is_superadmin and db_note.user_id and db_note.user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Note not found")
    return db_note
