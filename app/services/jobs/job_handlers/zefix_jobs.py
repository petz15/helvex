...
def handle_re_geocode(ctx: JobContext) -> tuple[dict, str]:
    from app.services.ingestion.collection import re_geocode_all_companies
...
def handle_recalculate_scores(ctx: JobContext) -> tuple[dict, str]:
    from app.services.ingestion.collection import recalculate_flex_scores
...
def handle_recalculate_google_scores(ctx: JobContext) -> tuple[dict, str]:
    from app.services.ingestion.collection import recalculate_google_scores
...
def handle_reextract_purpose(ctx: JobContext) -> tuple[dict, str]:
    from app.services.ingestion.collection import reextract_purpose_from_zefix_raw
...
def handle_bulk(ctx: JobContext) -> tuple[dict, str]:
    from app.services.ingestion.collection import bulk_import_zefix
    from app import crud
...
def handle_batch(ctx: JobContext) -> tuple[dict, str]:
    ...
    from app.services.enrichment.web_enrichment import _active_provider
    provider = _active_provider(ctx.db)
    done_msg = (
        f"Done [{provider}] — {stats['google_enriched']} enriched, "
        f"{stats.get('google_pending_crawl', 0)} pending crawl, "
...
def handle_initial(ctx: JobContext) -> tuple[dict, str]:
    from app.services.ingestion.collection import initial_collect
...
def handle_reextract_zefix_raw(ctx: JobContext) -> tuple[dict, str]:
    from app.services.ingestion.zefix_import import reextract_zefix_raw_fields, REEXTRACTABLE_FIELDS
...
def handle_detail(ctx: JobContext) -> tuple[dict, str]:
    from app.services.ingestion.collection import run_zefix_detail_collect
