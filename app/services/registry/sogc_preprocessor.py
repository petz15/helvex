...
def run_sogc_preprocess_batch(
    ...
    *,
    ...
    # The NOT EXISTS anti-join count is only for progress display — the batch
    # loop below re-applies the same filter per page via cursor pagination and
    # doesn't need an exact total. Bump statement_timeout for this one query
    # (mirrors app/services/noga_pipeline.py) and fall back to a cheap
    # planner estimate rather than failing the whole job if it's still slow.
    try:
        ...
        db.rollback()
        total = db.execute(
            text("SELECT reltuples::bigint FROM pg_class WHERE relname = 'companies'")
        ).scalar() or 0
    stats["selected"] = total
