...
from app.services.ingestion.collection import enrich_company, import_company_from_zefix_uid
...
def import_shab_publications(
    ...
    *,
    ...
    for i, pub in enumerate(publications[start:], start=start + 1):
        ...
        try:
            if sub_rubric in (SUBR_NEW, SUBR_MUTATION):
                ...
                try:
                    from app.services.registry.sogc_preprocessor import preprocess_company_sogc_pub
                    preprocess_company_sogc_pub(db, company)
                except Exception:
                    logger.warning("SOGC preprocess failed for uid=%s", uid, exc_info=True)

                try:
                    from app.services.registry.sogc_person_extractor import extract_persons_for_publication
    ...
    if new_uids and app is not None:
        try:
            from app.services.jobs.job_worker import enqueue_job, kick_job_worker
