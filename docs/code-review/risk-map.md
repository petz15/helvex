...
| `apply_successful_payment` | `app/services/payment_transactions.py:304` | Sets `org.tier` + grants `upgrade_proration_credits` for Worldline subscriptions — proration logic has no Stripe equivalent (`apply_subscription_update`). If the upgrade bug is server-side, this asymmetry is the first place to look. |
...
|---|---|---|---|
| `_run_job` | 394 | `app/services/job_worker.py:323` | The job dispatcher. 1059-line single function (noqa C901 — complexity check already suppressed). Its inline `elif` chain silently shadows most of the `JOB_HANDLERS` registry — see Priority 1 and the job-system deep-dive. |
...
| `run_batch_collect` | 192 | `app/services/web_enrichment.py` | Core Google-search enrichment loop. |
| `_progress` | 189 | `app/services/job_worker.py` | Progress-callback closure inside `_run_job` — tightly coupled to the god function above. |
...
| `claude_classify_batch` | 128 | `app/services/claude_classify.py` | AI scoring batch job — costs real money per call (~$0.25/1k companies per CLAUDE.md); a bug here has a $ blast radius, not just a correctness one. |
...
- `app/services/job_worker.py::enqueue_job`, `_enqueue_job_in_session`
...
| `app/services/job_worker.py` | 1784 (incl. `_run_job` at 1059) |
| `app/services/cluster_pipeline.py` | 1422 |
| `app/api/routes/admin/__init__.py` | 1407 |
| `app/crud/company.py` | 1317 |
| `app/services/job_handlers/web_crawl.py` | 1303 |
| `app/services/zefix_import.py` | 1257 |
| `app/services/scoring.py` | 1256 |
| `app/services/sogc_person_extractor.py` | 1166 |
| `app/services/shab_archive_import.py` | 1128 |
| `app/services/payments/worldline_provider.py` | 760 (`WorldlineProvider` class alone: 521) |
...
- **Two stale "Redis" comments fixed in passing** (`app/auth.py`,
  `app/services/providers/gemini.py`) — both described Redis-backed paths
