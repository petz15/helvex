...
| Worldline subscription webhook | `app/api/routes/billing/webhooks.py::worldline_return` (~375 lines, in-route business logic) | High blast radius; **now HTTP-tested** in `tests/test_billing_routes.py` (the graph's "untested" flag was a false negative — tests drive it over `TestClient`). Thin spots remain: the Payment Page `assert_payment_page` branch and proration. `ROADMAP.md` lists "existing subscription then upgrading is not working" — this is the exact codepath. See deep-dive. |
| `apply_successful_payment` | `app/services/billing/payment_transactions.py:304` | Sets `org.tier` + grants `upgrade_proration_credits` for subscriptions (Worldline-only). The proration credit-inflation risk is **fixed** (2026-07): the grant is now recomputed server-side via `compute_upgrade_proration_credits` and the client request field was dropped, so the value on the pending row is server-written, not client-supplied. Still the first place to look for the "upgrade doesn't work" bug. |
