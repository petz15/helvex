...
| `apply_successful_payment` | `app/services/billing/payment_transactions.py:304` | Sets `org.tier` + grants `upgrade_proration_credits` for subscriptions. Worldline is the only provider now, but `upgrade_proration_credits` is still read verbatim off the pending row (client-supplied, not server-recomputed, not covered by the entitlement re-derivation guard) — first place to look for the upgrade bug / a credit-inflation risk. |
