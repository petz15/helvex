...
- PRIO! improve the webscoring by not scoring the available url from serper until I have actually scraped it and compared it to the other results! this needs a general overhaul over the current methodology! -> **design approved:** [`docs/code-review/web-identity-rework.md`](docs/code-review/web-identity-rework.md) — pre-crawl snippet score demoted to crawl-ordering only; identity decided post-crawl from an evidence ledger with categorical outcomes (incl. AMBIGUOUS auto-pick, RELATED_ENTITY, UNKNOWN).
...
- [x] **Web crawler — external paid scrape fallback tier** — Done (2026-08-03): `tier='external'` reached only from a Playwright bot-block (`escalate_to_external`), driven by `web_crawl_external` via `clients/scrapingdog_scrape_client.py`. Identity phase only, never auto-enqueued, bounded by `limit`/`_EXTERNAL_MAX_PER_RUN`/2 pages per company. Superseded detail below:
- [x] ~~**Web crawler — external paid scrape fallback tier**~~ — a 3rd `tier=external` last resort for hard Cloudflare/CAPTCHA sites that defeat both httpx (curl_cffi impersonation) and Playwright. Reuse the existing ScrapingDog integration pattern (`scrapingdog_search_client.py`); gate by org/credits so cost is bounded to the few sites that need it. Deferred from the phase-2 bot-protection build.
...
- [ ] **Web identity & scoring rework** (approved design — supersedes the "URL confidence GBM" item below) — split the overloaded `web_score`/`confidence` scalar into **identity** (probability + category + evidence ledger; ledger-first, GBM later), **content** (global facts), and **fit** (per-org, in the scoring rework). Pre-crawl snippet score → crawl-ordering only; categorical outcomes (MATCH_UID/STRONG/WEAK, AMBIGUOUS auto-pick, RELATED_ENTITY, MISMATCH, NO_SITE, UNKNOWN); UID hardened (CHE checksum + related-entity graph lookup); `web_score` retired from `combined_score`. 6-phase migration. Full design: [`docs/code-review/web-identity-rework.md`](docs/code-review/web-identity-rework.md).
- [ ] **Web extract — URL confidence: trained logistic regression / GBM model** (folded into the rework above as its phase-6 "model later" stage) — replace the hand-tuned additive weights in `crawler_extract.resolve_company_extract` with a logistic regression or gradient-boosted model trained on labeled company→site pairs. Features already computed: `uid_score`, `addr_score`, `zone_name_conf`, `base` (signal coverage), **`purpose_sim`** (now stored in `company_web_extract` by `enrich_web_purpose_sim`). Bootstrap labels by exporting companies with confidence 0.25–0.65 (the uncertain middle band) and classifying them manually; ~500 pairs sufficient for sklearn. The evidence ledger built in the rework's phase 1 IS this model's feature vector, and its categorical outcomes are passive training labels. Serialize trained model to `app/services/url_confidence_model.joblib`; fall back to the deterministic ledger if model file absent.
- [ ] **Google results & scoring** — Improve the selection and scoring of google results -> covered by the web identity & scoring rework ([`docs/code-review/web-identity-rework.md`](docs/code-review/web-identity-rework.md)): Google snippet scoring becomes crawl-ordering only, identity decided from post-crawl evidence.
