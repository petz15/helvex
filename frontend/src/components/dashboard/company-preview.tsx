...
export function CompanyPreview({ company: incoming, onClose, onUpdated, className }: CompanyPreviewProps) {
  const { dict } = useI18n();
  const t = dict.app.companyPreview;
    ...
    <aside className={cn("shrink-0 flex flex-col bg-white border-l border-slate-200 overflow-y-auto", className ?? "w-80")}>
          ...
          <div className="min-w-0">
            <h2 className="font-semibold text-slate-800 text-sm leading-snug break-words flex items-center gap-1 flex-wrap">
              ...
              {company.zefix_detail_web && (
                <a href={company.zefix_detail_web} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-blue-600 shrink-0" title={t.viewOnZefix}>
      ...
      <div className="px-4 py-2 flex flex-wrap gap-1.5 border-b border-slate-100">
        <Badge className={reviewBadgeClass(company.review_status)}>
          {company.review_status?.replace(/_/g, " ") ?? t.pending}
        </Badge>
        ...
        {company.tags && company.tags.split(",").map((tag) => (
          <Badge key={tag.trim()} className="bg-slate-100 text-slate-600">{tag.trim()}</Badge>
        ))}
      </div>
      ...
      <div className="px-4 py-3 flex flex-col gap-2 border-b border-slate-100">
        <div className="flex items-center justify-between text-xs">
          <span className="text-slate-700 font-medium w-16">{t.combined}</span>
          <ScoreBar score={company.combined_score} className="flex-1" />
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="text-slate-700 font-medium w-16">{t.web}</span>
          <ScoreBar score={company.web_score} className="flex-1" />
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="text-slate-700 font-medium w-16">{t.ai}</span>
          <ScoreBar score={company.ai_score} className="flex-1" />
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="text-slate-700 font-medium w-16">{t.flex}</span>
        ...
        {company.flex_score_breakdown && (() => {
          try {
            ...
            const entries: [string, number][] = [
              [t.clusters, bd.clusters], [t.keywords, bd.keywords],
              [t.distance, bd.distance], [t.legalForm, bd.legal_form],
      ...
      <div className="px-4 py-3 flex flex-col gap-2 text-sm border-b border-slate-100">
        {company.website_url && (
          <div>
            <span className="text-xs font-medium text-slate-600 block mb-0.5">{t.website}</span>
        ...
        {(company.address || company.address_city || company.address_zip) && (
          <div>
            <span className="text-xs font-medium text-slate-600 block mb-0.5">{t.address}</span>
        ...
        {company.purpose && (
          <div>
            <span className="text-xs font-medium text-slate-600 block mb-0.5">{t.purpose}</span>
            <p className="text-xs text-slate-700 line-clamp-4">{company.purpose}</p>
          </div>
        )}
        {company.ai_category && (
          <div>
            <span className="text-xs font-medium text-slate-600 block mb-0.5">{t.aiCategory}</span>
        ...
        {company.translations && (() => {
          try {
            ...
            if (names.length > 0) {
              return (
                <div>
                  <span className="text-xs font-medium text-slate-600 block mb-0.5">{t.alsoKnownAs}</span>
                  <div className="flex flex-wrap gap-1">
                    {names.map((name, i) => (
                      <Badge key={i} className="bg-slate-100 text-slate-600 text-xs">{name}</Badge>
                    ))}
                  </div>
                </div>
              );
        ...
        {(company.contact_name || company.contact_email || company.contact_phone) && (
          <div>
            <span className="text-xs font-medium text-slate-600 block mb-0.5">{t.contact}</span>
        ...
        <div className="flex gap-4">
          <div>
            <span className="text-xs text-slate-400 block">{t.updated}</span>
      ...
      {company.notes.length > 0 && (
        <div className="px-4 py-3 border-b border-slate-100">
          <span className="text-xs font-semibold text-slate-700 block mb-2">{t.notes.replace("{count}", String(company.notes.length))}</span>
      ...
      <div className="px-4 py-3 mt-auto">
        ...
        >
          {t.openFullProfile} <ChevronRight size={15} />
        </Link>
      </div>
    </aside>
  );
