...
function OAuthButtons({ orSignUpWith }: { orSignUpWith: string }) {
  return (
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (body.detail === "email_unverified") { setShowResend(true); setError(null); return; }
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="w-full max-w-sm rounded-xl border border-slate-200 bg-white p-8 shadow-sm text-center">
          ...
          <p className="text-xs text-slate-400 mb-4">Don&apos;t see it? Check your spam or junk folder.</p>
          {inviteToken && (
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center">
      <div className="w-full max-w-sm rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        {showResend ? (
          <div className="space-y-3">
            ...
            {resendDone && resendCooldown > 0 && (
              <p className="text-xs text-slate-400 text-center">You can request another in {resendCooldown}s</p>
            )}
            <button
              type="button"
        ...
        {!showResend && <OAuthButtons orSignUpWith={t.orContinueWith} />}

        <p className="mt-4 text-center text-sm text-slate-500">
          {t.hasAccount}{" "}
          <Link
            href={inviteToken ? `/${locale}/accept-invite?token=${inviteToken}` : `/${locale}/login`}
